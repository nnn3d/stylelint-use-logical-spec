...
const physicalShorthandPropLogicalKeyword = ['margin', 'padding'];
        ...
        if (!isLogicalKeyword) {
          ...
          physicalShorthandProp.forEach(prop => {
            validateRuleWithProps(node, [prop], physicalDecl => {
              // eslint-disable-line
              let inputValues = valueParser__default['default'](physicalDecl.value).nodes.filter(value => value.type !== 'space').map(value => valueParser__default['default'].stringify(value));
